 ...
 *
 * Why this module: shared.ts was a shallow grab-bag (256 lines) owning HeadWriter + HEAD_EXIT_CODES + protocolSnapshot (SnapshotView pagination + Effect.runSync log side-effect) + runHeadBoundary + finalAssistantText, each with an interface as complex as its implementation. Every Head (print, json, rpc) re-derived wire encoding independently — json.ts did Schema.encode(SnapshotSchema), print.ts extracted finalAssistantText, rpc-session-bridge called protocolSnapshot again. Bugs (exit-code mapping, snapshot warning log, Schema encode) hid between the helpers, and adding a Snapshot audit field touched all three Heads plus shared.ts — 4 hops for one concept.
 *
